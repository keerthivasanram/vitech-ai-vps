...
def _row2(pdf, w1, t1, w2, t2, bold2=False, h=5.5):
    ...
    rh = max(lh.measure_h(pdf, w1, t1, 9, line_h=h),
             lh.measure_h(pdf, w2, t2, 9, style="B" if bold2 else "", line_h=h))
...
def render_quotation_pdf(quote: dict) -> bytes:
    ...
    if terms:
        ...
        for t in terms:
            ...
            rh = max(lh.measure_h(pdf, 40, k, 9, style="B"),
                     lh.measure_h(pdf, usable - 40, v, 9))
            lh.ensure_space(pdf, rh + 2)
            x, y0 = pdf.l_margin, pdf.get_y()
